...
using namespace std; 
//max, uninitialized_fill, uninitialized_copy,allocator
...
template <class T> void Vec<T>::create(const_iterator i, const_iterator j) {
	...
	limit = avail = uninitialized_copy(i, j, data); //allocator 비멤버
}
...
template <class T> void Vec<T>::uncreate() {
	if (data) {
		...
		while (it != data) //첫 번째 요소의 가리키는 포인터까지
			alloc.destroy(--it); //it감소하기
...
template <class T> void Vec<T>::grow() {
	//저장 공간을 늘릴 대는 현재 사용하는
	//저장 공간의 2배를 할당
	size_type new_size = max(2 * (limit - data), ptrdiff_t(1));

	//새로운 저장 공간을 할당하고 기존 요소들을 개로운 저장 공간에 복사
